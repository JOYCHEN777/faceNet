...
from util import set_random_seeds, create_model, prepare_dataloader, train_model, save_model, load_model, \
    evaluate_model, create_classification_report
...
def main():
    ...
    model_dir = "saved_models"
    model_filename = "facenet_mobilenet.pth"
    model_filename_prefix = "pruned_model"
    pruned_model_filename = "resnet18_pruned_webnet.pt"
    model_filepath = os.path.join(model_dir, model_filename)
    ...
    train_loader, test_loader, classes = prepare_dataloader(
        num_workers=8, train_batch_size=64, eval_batch_size=64)
